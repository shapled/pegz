...
pub fn main() !void {
    ...
    // Create output writer with larger buffer for large generated files
    ...
    // Use larger buffer (1MB) for large generated parser files
    ...
    // Read entire file
    const file_size = in_file.getEndPos() catch |err| {
        std.log.err("Error getting file size: {}\n", .{err});
        std.process.exit(6);
    };
